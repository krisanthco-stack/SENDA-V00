...
def _wait_for_health(url: str, timeout: float = 15.0) -> None:
    ...
    while time.monotonic() < deadline:
        try:
            ...
            last_error = exc
        time.sleep(0.15)
    raise RuntimeError(f'El servidor no superó la verificación de salud: {last_error}')


def run(host='127.0.0.1', port=8765, *, data_dir=None, ui_dir=None, no_browser=False) -> int:
    server = None
    thread = None
    logger = None
    log_path = None
    try:
        ...
        logger.info('Servidor verificado: %s%s', url, HEALTH_PATH)
...
def main(argv=None) -> int:
    ...
    return run(args.host, args.port, data_dir=args.data_dir, ui_dir=args.ui_dir, no_browser=args.no_browser)
