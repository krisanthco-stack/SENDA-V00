...
import csv, hashlib, io, itertools, json, os, re, shutil, subprocess, sys, tempfile, unicodedata, zipfile
...
def _convert_xls_to_csv(path:Path, outdir:Path) -> Path:
    # First use xlrd when a deployment provides it.
    try:
...
def normalize_row(row:dict, *, source:str, year:int, quarter:str, district:str, source_file:str, ctx:dict):
    ...
    return {'folio':_folio(r),'derecho':right,'plano':clean(r.get('NUM_PLANO') or r.get('PLANO')),'fecha':d.isoformat() if d else '',
            'codigo':full,'operacion':op,'tipo':source,'fuente':source,'cedula':clean(r.get('NUMERO_IDENT') or r.get('CEDULA') or r.get('CEDULAJURIDICA')),
...
def extract_rar(path:Path,target:Path):
    ...
    try:
        ...
        for tool,args in [('7z',['x','-y',f'-o{target}',str(path)]),('7za',['x','-y',f'-o{target}',str(path)]),('unrar',['x','-o+',str(path),str(target)]),('tar',['-xf',str(path),'-C',str(target)])]:
            exe=shutil.which(tool)
            if exe:
                cp=subprocess.run([exe,*args],stdout=subprocess.PIPE,stderr=subprocess.PIPE,timeout=600)
                if cp.returncode==0:return
        raise RuntimeError('RAR reconocido, pero no hay extractor RAR disponible (7-Zip/WinRAR/unrar/tar).') from first
            ...
            with ExitStack() as stack:
                ...
                for p in expanded:
                    ...
                    try:
                        ...
                        source=detect_source(p,first.keys());
                        def normalized():
                            nonlocal skipped
                            for raw in itertools.chain((first,),it):
                                x=normalize_row(raw,source=source,year=year,quarter=quarter,district=district,source_file=p.name,ctx=ctx)
